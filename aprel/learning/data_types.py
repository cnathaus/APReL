...
:TODO: OrdinalQuery classes will be implemented so that the library will include ordinal data, which was used for
reward learning in:
...
class PreferenceQuery(Query):
    """
    A preference query is one where the user is presented with multiple trajectories and asked for their favorite among
    them.
...
class WeakComparison(QueryWithResponse):
    ...
    def __init__(self, query: WeakComparisonQuery, response: int):
        ...
        assert response in self.query.response_set, (
            "Invalid response " + str(response) + " for the weak comparison query."
        )
        self.response = response
...
class FullRankingQuery(Query):
    """
    A full ranking query is one where the user is presented with multiple trajectories and asked for a ranking
    from their most
    ...
    Attributes:
        K (int): The number of trajectories in the query.
        response_set (numpy.array): The set of possible responses to the query, which is all :py:attr:`K`-combinations
        of the
        ...
        while i < self.K:
            ...
            while selection is None:
                ...
                if (
                    ...
                    print(
                        "You have already chosen trajectory " + selection + " before!"
                    )
                    selection = None
