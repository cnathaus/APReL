...
class QueryOptimizerDiscreteTrajectorySet(QueryOptimizer):
    ...
    def optimize(
        ...
        **kwargs
        ...
        assert batch_size > 0 and isinstance(
            batch_size, int
    ...
    def exhaustive_search(
        self, acquisition_func: Callable, belief: Belief, initial_query: Query, **kwargs
    ) -> Tuple[List[Query], np.array]:
        """
        ...
        **kwargs
        ...
        if (
            ...
            if acquisition_func is random:
                ...
                print("optimal query: ", subsets[inds[0]])
                ...
                for ids in subsets:
                    ...
                    vals.append(
                        acquisition_func(weights, belief_logprobs[ids], **kwargs)
                    )
                vals = np.array(vals)
                inds = np.argpartition(vals, -batch_size)[-batch_size:]
                ...
                for ids in subsets:
                    ...
                    vals.append(
                        acquisition_func(
                            weights, belief_logprobs[ids], planned_trajs[ids], **kwargs
                        )
                    )
                vals = np.array(vals)
                inds = np.argpartition(vals, -batch_size)[-batch_size:]
