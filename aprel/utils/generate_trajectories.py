...
from aprel.basics import Environment, Trajectory, TrajectoryGym, TrajectorySet
...
def generate_trajectories_randomly_gym(
    env: Environment,
    num_trajectories: int,
    max_episode_length: int = None,
    file_name: str = None,
    ...
    if trajectories.size >= num_trajectories:
        ...
        for traj_no in range(trajectories.size, num_trajectories):
            ...
            if env_has_rgb_render:
                ...
                clip_path = None
            trajectories.append(TrajectoryGym(env, traj, clip_path))
            # if env.close_exists:
            #     env.close()
