...
def update_belief_inquire(queries, responses, belief, belief_opt, true_user):
    ...
    selected_phis = [
        ...
        np.expand_dims(np.array(fb.query.slate.features_matrix[fb.response]), axis=0)
        for fb in queries_with_responses
    ]
    ...
    print("selected_phis: ", selected_phis)
    print("comp_phis: ", comp_phis)
...
def main():
    ...
    trajectories = [
        aprel.TrajectoryGym(env=env, trajectory=[np.random.rand(feature_dims), None])
    ...
    if not os.path.exists("experiment_plots/parameter_estimation_3"):
        os.makedirs("experiment_plots/parameter_estimation_3")
    if not os.path.exists("experiment_plots/parameter_estimation_3/num_queries"):
        os.makedirs("experiment_plots/parameter_estimation_3/num_queries")

    foldername = "experiment_plots/parameter_estimation_3/num_queries/"
    ...
    np.savetxt(
        foldername + "cos_sim_parameter_estimation_3.txt",
        data,
        header=header,
        delimiter="\t",
        fmt="%.4f",
    ...
    for i in range(len(mean)):
        ...
        # plt.show()
        plt.savefig(foldername + f"cos_sim_parameter_estimation_3_{i}.png", dpi=300)
